...
void loop() {
  ...
  if (now - lastPrintTime > 500) { // 每500ms检查一次
    lastPrintTime = now;

    if (digitalRead(PRESSURE_1_PIN) == LOW) {
      Serial.println("PRESSURE 1 pressed");
    }
    if (digitalRead(PRESSURE_2_PIN) == LOW) {
      Serial.println("PRESSURE 2 pressed");
    }
    if (digitalRead(PRESSURE_3_PIN) == LOW) {
      Serial.println("PRESSURE 3 pressed");
    }
  }
  ...
  bool currentPressureState = allPressureSensorsPressed();

  if (currentPressureState && !lastPressureState) {
    if (running && !systemJustRestarted) {
      Serial.println("All sensors pressed. System stopping and resetting...");
      stopAndReset();
      restartSystem();
      systemJustRestarted = true;
    } else if (running && systemJustRestarted && roundNumber == 1) {
      Serial.println("All sensors pressed again. Starting Round 1.");
      resetAllStrips();
      FastLED.show();
      selectParticipant();
      systemJustRestarted = false;  // 一旦第一轮开始，状态归位
    }
    delay(200); // 消抖
  }
